...
- packages 资产：漂移门禁（`ci/script/check_package_drift.py`）核对 `app/src/main/assets/packages/` 与 `tools/example_packages/upstream_pin.json` 逐件一致，并监测上游 dev 分支是否前进（见 `docs/TODO/upstream_draw_sync_20260926/`）。
